...
from utils.torch_utils import fuse_conv_and_bn, initialize_weights, model_info, scale_img, \
...
class Model(nn.Module):
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            b.data[:, 5:] += math.log(0.6 / (m.nc - 0.999999)) if cf is None else torch.log(
                cf / cf.sum())  # cls
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus,
                 CrossConv, BottleneckCSP, C3, C3TR, C3Ghost, NextConv, Shuffle_Block, Stem, CF2,
                 MC, C3SPPF]:
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost, CF2]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./yolov5s-im3.yaml', help='model.yaml')
    ...
    im = torch.rand(1 if torch.cuda.is_available() else 1, 3, 640, 640).to(device)
    torch.cuda.synchronize()
    t0 = time.time()
    for _ in range(100):
        model(im)
    torch.cuda.synchronize()
    print(f'延迟:{(time.time() - t0) / 100 * 1000:.4f}ms')
