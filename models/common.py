...
class SN(nn.Module):
    def __init__(self, dim, g=4):
        super().__init__()
        self.cv1 = Conv(dim, dim, 3, 1, g=g)
        self.cv2 = Conv(dim, dim, 3, 1, g=g)
...
class CF2(nn.Module):
    def __init__(self, c1, c2, n=1, e=0.5, g=4):
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, self.c * 2, 1, 1)
        self.cv2 = Conv((1 + n) * self.c, c2, 1)
        self.m = nn.ModuleList([SN(self.c, g) for _ in range(n)])
...
class ESA(nn.Module):
    def __init__(self, c=192, kernel_size=7):
    ...
    def forward(self, x):
        return x * self.act(self.cv1(
            torch.cat([torch.mean(x, 1, keepdim=True), torch.max(x, 1, keepdim=True)[0]], 1)))
...
class MG(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.s3 = ESA(c1)
        self.cat = Concat()
        self.fc = nn.Linear(c1 * 3, c1 // 2)
        self.fcs = nn.ModuleList([])
        for i in range(3):
            self.fcs.append(nn.Linear(c1 // 2, c1))
    ...
    def forward(self, x):
        ...
        a1, a2, a3 = scores[:, 0, ...], scores[:, 1, ...], scores[:, 2, ...]
        x = a1 * x1 + a2 * x2 + a3 * x3
        return x
...
class PConv(nn.Module):
    def __init__(self, dim, n_div):
        ...
        self.cv2 = nn.Conv2d(dim, dim, 1, bias=False)
    ...
    def forward(self, x):
        ...
        x = torch.cat((x1, x2), 1)
        x = shortcut + self.cv2(self.cv1(x))
        return x


# build DPConv
# -------------------------------------------------------------------------
class NextConv(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, s=1):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = nn.Sequential(Conv(c1, c2, 1),
                                  DWConv(c2, c2, 7, s),
                                  Conv(c2, c2, 1))
...
class Stem(nn.Module):
    def __init__(self, c1, c2, k=3, s=2):  # ch_in, ch_out
        super().__init__()
        c_ = c2 // 2
        self.conv1 = Conv(c1, c_, k, s)

        self.conv2 = nn.Sequential(Conv(c_, c_, 1),
                                   Conv(c_, c2, 3, 2))
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)
        self.conv3 = Conv(c_ + c2, c2, 1)
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=None, dnn=False, data=None):
        ...
        if pt:  # PyTorch
            ...
            network = core.read_network(model=w,
    ...
    def forward(self, im, augment=False, visualize=False, val=False):
        ...
        if self.pt or self.jit:  # PyTorch
            ...
            desc = self.ie.TensorDesc(precision='FP32', dims=im.shape,
                                      layout='NCHW')  # Tensor Description
            request = self.executable_network.requests[0]  # inference request
            request.set_blob(blob_name='images',
                             blob=self.ie.Blob(desc, im))  # name=next(iter(request.input_blobs))
            request.infer()
    ...
    def warmup(self, imgsz=(1, 3, 640, 640), half=False):
        ...
        if self.pt or self.jit or self.onnx or self.engine:  # warmup types
            if isinstance(self.device,
                          torch.device) and self.device.type != 'cpu':  # only warmup GPU models
    ...
    def model_type(p='path/to/model.pt'):
        ...
        pt, jit, onnx, xml, engine, coreml, saved_model, pb, tflite, edgetpu, tfjs, xml2 = (s in p
                                                                                            for s in
                                                                                            suffixes)
...
class AutoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        autocast = self.amp and (
                p.device.type != 'cpu')  # Automatic Mixed Precision (AMP) inference
        ...
        shape1 = [make_divisible(x, self.stride) for x in
                  np.stack(shape1, 0).max(0)]  # inference shape
        x = [letterbox(im, new_shape=shape1 if self.pt else size, auto=False)[0] for im in
             imgs]  # pad
        ...
        with amp.autocast(enabled=autocast):
            ...
            # Post-process
            y = non_max_suppression(y if self.dmb else y[0], self.conf, iou_thres=self.iou,
                                    classes=self.classes,
                                    agnostic=self.agnostic, multi_label=self.multi_label,
...
class Detections:
    ...
    def display(self, pprint=False, show=False, save=False, crop=False, render=False,
                save_dir=Path('')):
        crops = []
            ...
            if pred.shape[0]:
                ...
                if show or save or render or crop:
                    ...
                    for *box, conf, cls in reversed(pred):  # xyxy, confidence, class
                        ...
                        if crop:
                            file = save_dir / 'crops' / self.names[int(cls)] / self.files[
                                i] if save else None
    ...
    def tolist(self):
        ...
        x = [Detections([self.imgs[i]], [self.pred[i]], [self.files[i]], self.times, self.names,
                        self.s) for i in r]
        # for d in x:
